...
class OpenId {
  ...
  public getToken(code) {
    return `${this.config.Oauth}/access_token?appid=${
      this.config.Appid
    }&secret=${this.config.Secret}&code=${code}&grant_type=authorization_code`;
  }
  // 刷新access_token（如果需要）
  public refreshToken(refreshToken) {
    return `${this.config.Oauth}/refresh_token?appid=${
      this.config.Appid
  ...
  public userInfo(token, openid) {
    return `${
      this.config.Oauth
    }/userinfo?access_token=${token}&openid=${openid}&lang=zh_CN`;
  }
