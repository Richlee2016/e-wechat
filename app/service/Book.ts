...
export default class Book extends Service {
  ...
  public async fetchFalls(start:number,count:number){
    const {ctx,app} = this
    const {xmBook} = app.config
    const opt = Object.assign(this.commonOpt,{headers:{'Cookie':xmBook.cookie}})
    try {
      const res= await ctx.curl(`${xmBook.prefix}/rock/book/recommend?start=${start}&count=${count}`,opt)
  ...
  /** 代理 */
  public async bookProxy(method:'GET'|'POST' = 'GET',url:string,data?:any){
    const {ctx,app} = this
    const {xmBook} = app.config
    let opt:any ={
      method,
  ...
  private _categoryHandle(data){
    ...
    const mapList = items => {
      return items.map(o => {
        const {category_id,children,fiction_count,label,new_image,new_image2} = o
        return {
          id:category_id,
          name:label,
          children,
