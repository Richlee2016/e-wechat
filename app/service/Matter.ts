...
export default class Matter extends Service {
  ...
  public async createMatter(create: CreateMaterial) {
    const { Wx: { WxMatterConfig } } = this.app.config;
    const { file, token, type, description } = create;
    const filtType = this._getType(file.mimeType);
    const fetchUrl = type ? WxMatterConfig.createMaterial(token, filtType) : WxMatterConfig.createMedia(token, filtType);
    // 传输格式
    const media = {
      value: file.buffer,
      options: {
    ...
    const opt = {
      ...
      url: fetchUrl,
      // 是否是永久视频素材
      formData: filtType === 'video' && type ? { media, description: JSON.stringify(description) } : { media },
      json: true
    };
    try {
